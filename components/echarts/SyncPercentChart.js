...
export default function SyncPercentChart({
    nodeId,
    isLoading,
    setIsLoading,
    fetching,
    ...
    useEffect(() => {
        ...
        setFetching(true);
        fetch(
            `http://chaindeck.api.io:30967/query_range_v1?query=eth_exe_sync_percentage{pod=~"$pod"}&start_time=${startDate}T${startTime}&end_time=${date}T${timeStamp}&step=300`,
            {
                method: "GET",
                headers: {
